...
$_lang['setting_magicpreview.click_to_field_desc'] = 'Experimental. When enabled, clicking a field in the preview scrolls the resource form to that field and highlights it. Works best with ContentBlocks, which wraps each field\'s output automatically. A best-effort attempt is also made to wrap core resource fields (pagetitle, longtitle, description, menutitle, introtext) and TVs via the parser, though this may not cover all template output combinations. Default: No.';
